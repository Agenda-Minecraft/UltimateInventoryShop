...
interface GoodsMapper {
    ...
    @Select("SELECT * FROM \${tableName} WHERE been_sold=#{beenSold} ORDER BY create_date DESC")
    fun getAllGoods(beenSold: Boolean=false, tableName: String = "${ConfigLoader.tablePrefix}goods"): List<GoodPojo>

    @Select("SELECT * FROM \${tableName} WHERE been_sold=#{beenSold} AND putter_uid=#{playerUid} ORDER BY create_date DESC")
    ...
    @Select("SELECT * FROM \${tableName} WHERE been_sold=#{beenSold} AND item_info=#{itemInfo} ORDER BY create_date DESC")
    ...
    @Select("SELECT COUNT(1) FROM \${tableName} WHERE been_sold=#{beenSold} ORDER BY create_date")
    fun getCount(tableName: String="${ConfigLoader.tablePrefix}goods", beenSold: Boolean=false): Int
    @Select("SELECT COUNT(1) FROM \${tableName} WHERE been_sold=#{beenSold} AND putter_name=#{putterName} ORDER BY create_date")
    fun getCountPlayer(tableName: String="${ConfigLoader.tablePrefix}goods", putterName: String, beenSold: Boolean=false): Int
